...
def save_img(image, width, height, path):
    io.print("Size: " + str(width) + "x" + str(height))
    io.print("Data points: " + str(len(image)))
    io.print("Path: " + path)
...
def raytracer(scene, output_path):
    io.print("[DEBUG]: Setting up raytracer...")
    width = scene.camera.width
    height = scene.camera.height
    io.print("[DEBUG]:   camera.dimensions[" + str(width) + "x" + str(height) + "]")
    io.print("[DEBUG]:   camera.exposure[" + str(scene.camera.exposure) + "]")
    io.print("[DEBUG]:   camera.fov[" + str(scene.camera.fov) + "]")
    ...
    io.print("[DEBUG]:   camera.up[" + up.repr() + "]")
    io.print("[DEBUG]:   camera.right[" + right.repr() + "]")
    io.print("[DEBUG]:   camera.forward[" + forward.repr() + "]")
    io.print("[DEBUG]:   camera.pos[" + root_pos.repr() + "]")
    io.print("[DEBUG]: Raytracing...")
    progress = 0
        ...
        while True:
            ...
            while True:
                ...
                while True:
                    ...
                    if ((progress & ((1<<22)-1)) == 0):
                        prog = progress / (width*height*PIXEL_SAMPLES*LENS_SAMPLES)
                        io.print("\x1b[96m\r\x1b[0K[PROGRESS]: " + str(prog) + "%\x1b[0m\r")
                    progress += 1
            image[y*width+x] = colour.div(PIXEL_SAMPLES*LENS_SAMPLES)
    io.print("\x1b[2K\x1b[1K\r\x1b[0K[DEBUG]: Applying tone map...")
    tone_map(image, width, height, scene.camera)
    io.print("[DEBUG]: Saving...")
    save_img(image, width, height, output_path)
    io.print("\x1b[96m[DONE]\x1b[0m")
...
def main():
    scene = load_scene()
    if scene:
        raytracer(scene, "img-python.ppm")
